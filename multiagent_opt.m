    ...
    while converged==false
        ...
        for k=1:numKs
            ...
            % that to the found merit of each paremeter value taken.
             ...
             if conviolr < bestconviol(e) %&& all(constraints <= 0.01)
                    bestconviol(e) = conviolr;
            ...
            if rewardstruct=='D'
              ...
              for compnum=1:length(comp)
                  
                  temps_count=temps;
                  temps_count(comp{compnum})=0*temps(comp{compnum});
                ...
                rewardsr_comp(compnum)=calc_rewards([learnedir,learnedcr],[objimprovementir,objimprovementcr],[conimprovementir,conimprovementcr],conscale, rewardtype,rewardstruct);
                rewardsc_comp(compnum)=calc_rewards([learnedic,learnedcc],[objimprovementic,objimprovementcc],[conimprovementic,conimprovementcc],conscale, rewardtype,rewardstruct);
                rewards_comp(compnum)=rewardsr_comp(compnum)-rewardsc_comp(compnum);
                
                rewards(comp{compnum})=rewards_comp(compnum);
